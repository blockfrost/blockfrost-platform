...
- `8081/tcp`: Prometheus metrics endpoint for monitoring (optional but recommended). Metrics can be disabled altogether if you prefer.
...
<Callout type="info">
  **On our roadmap**: We're developing a reverse-connection mechanism using WebSockets that will enable IP address filtering directly in the Blockfrost platform. This feature will simplify security in complex network environments (firewalls, double NAT, CGNAT) and eliminate the need for static IP addresses. Monitor updates on [GitHub issue #178](https://github.com/blockfrost/blockfrost-platform/issues/178) for NAT handling improvements.
</Callout>
...
- Access the Icebreaker dashboard at [http://icebreakers.blockfrost.io/](http://icebreakers.blockfrost.io/) to check the status of your node
...
Your node appears on the [Icebreaker dashboard](http://icebreakers.blockfrost.io/) as 'Icebreaker X' where 'X' is your assigned Icebreaker number.
...
The Icebreaker dashboard ([http://icebreakers.blockfrost.io/](http://icebreakers.blockfrost.io/)) provides real-time information about the Icebreaker network, including:
...
Passing `--no_metrics` in the cli or using the wizard to do so will disable the metrics endpoint and metrics gathering.

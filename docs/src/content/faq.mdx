...
- `/metrics` — Prometheus metrics (unless disabled with `--no-metrics`)
- `/tx/submit` — transaction submission (in [solitary mode](/usage/solitary) served at root, otherwise at `/{UUID}/tx/submit`)

When a [data node (Dolos)](/data-node) is connected, the platform also serves blockchain query endpoints including blocks, transactions, accounts, addresses, assets, pools, epochs, governance, metadata, network info, and more.

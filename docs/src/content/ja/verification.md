...
ローカルの Unix ソケット経由でプラットフォームから到達可能なメインネット (本番) cardano-node インスタンスが稼働していること。
...
プラットフォームは Blockfrost のロードバランサーへ WebSocket で**アウトバウンド**接続するため、Blockfrost 向けに着信ポートを開放する必要はありません。
アウトバウンドのインターネット接続が許可されていることを確認してください。
ローカルの Blockfrost プラットフォームのデフォルトポートは `3000` ですが、これは Prometheus メトリクスエンドポイントを自分でスクレイピングする場合にのみ到達できれば十分です。

#### クラウドホスティングの場合

- Azure: 送信ルールでインターネットへの HTTPS/WebSocket トラフィックを許可

- AWS: セキュリティグループで送信の HTTPS/WebSocket トラフィックを許可

- その他のプロバイダー: ファイアウォール設定でアウトバウンド接続が許可されていることを確認

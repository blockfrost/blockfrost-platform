...
まだ若干名の枠が残っています。
[Discord](https://discord.gg/YfGKvqpt2d) の `#ask-blockfrost` チャンネルでお気軽にお声がけください。
...
2. `blockfrost-platform` API はこれまでどおりポート 3000 で動作しますが、外部に公開する必要はありません。Blockfrost のトラフィックはすべて、ロードバランサーへのアウトバウンドの WebSocket 接続経由で届きます。

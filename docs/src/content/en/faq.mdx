...
<Callout type="default" emoji="📣">
  ...
  documentation? Join our community on [Discord](https://discord.gg/YfGKvqpt2d)
  in the `#ask-blockfrost` channel.
</Callout>
...
1. Connect with us on [Discord](https://discord.gg/YfGKvqpt2d) in the `#ask-blockfrost` channel to ask about receiving an Icebreaker NFT and a team member will assist you.
...
3. Reach out on [Discord](https://discord.gg/YfGKvqpt2d) in the `#ask-blockfrost` channel where a team member will assist you.
...
- Notify the Blockfrost team via [Discord](https://discord.gg/YfGKvqpt2d) in the `#ask-blockfrost` channel that you've received your NFT
...
If you've received your NFT but don't have channel access yet, post a message on [Discord](https://discord.gg/YfGKvqpt2d) in the `#ask-blockfrost` channel for assistance.
...
The Icebreaker node connects **outbound** to the Blockfrost load balancers over WebSocket and serves API requests over those connections, so it no longer needs any inbound port open to the public Internet.

Your firewall only needs to allow:

- **Outbound** connections to the Blockfrost load balancers (and to your Cardano node / data node).
- Optionally, access to the local Prometheus metrics endpoint on `3000/tcp` (or your custom port) from wherever you scrape it. This is only required if you monitor the node with Prometheus; metrics can be disabled altogether if you prefer.

Because API traffic now arrives over the outbound WebSocket connection, you do not need to expose port `3000` to the Internet or configure inbound firewall rules for Blockfrost.
...
No. The node connects outbound to the Blockfrost load balancers over WebSocket, so it works from behind firewalls, NAT, double NAT, or CGNAT, and does not require a static public IP or any inbound port.

A stable outbound Internet connection is all that is needed. Dynamic public IPs are fine.

### What should I do if my public IP address changes frequently?

A changing public IP is not a problem. Because the node establishes the connection to the Blockfrost load balancers itself, over an outbound WebSocket, it re-registers automatically whenever the connection drops and reconnects regardless of your current IP address.
...
3. Blocked outbound connectivity to the Icebreakers API or load balancers.
...
4. If you are not already in the Icebreakers private Discord channel, post a message on [Discord](https://discord.gg/YfGKvqpt2d) in the `#ask-blockfrost` channel for assistance, as other participants may have found solutions to similar problems.
...
- **Firewall:** Ensure your firewall allows outbound connections to the Icebreakers API and load balancers.
...
This error usually indicates that the node could not reach the Icebreakers API, or that its credentials are incorrect.
To resolve it:

1. Confirm your `--secret` and `--reward-address` are correct.
2. Ensure the node can make **outbound** connections to the Icebreakers API (check firewall/proxy rules for outgoing traffic).
3. Verify outbound connectivity by running `curl -v https://icebreakers-api.blockfrost.io/` from the node.
...
No.
Current versions serve the API directly at `/`, without the random UUID path prefix, so these warnings no longer appear.
On older versions they were harmless: each restart generated a new random UUID path, and the Blockfrost servers could briefly attempt to contact the old one.
...
<Callout type="info">
  The node connects to the Blockfrost load balancers over an outbound WebSocket
  and does not expose any inbound port to the public Internet. This keeps it
  usable in complex network environments (firewalls, double NAT, CGNAT) without
  a static public IP.
</Callout>

...
1. Both sides exchange `Ping` and `Pong` messages every 13 seconds, which allows us to detect the other side going offline quite quickly, even with silently dropped packets. And also to measure the pure network round-trip time, without the overhead of any advanced calculations. If a ping response is not received within the timeout, the connection is reset and re-registration is triggered.

...
Each participating Cardano node operator installs the Blockfrost Platform software, which allows them to join the Blockfrost backend fleet and offer their resources. In return, they earn revenue from the data services they provide.
...
Generally speaking, the first iteration of the network looks like below.

![Icebreakers network diagram](/icebreakers_network_diagram.png)

## How to join?

We have still some places left, so just around in the Discord channel.

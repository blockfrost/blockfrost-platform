...
use pallas_network::miniprotocols::{self, localstate};
...
pub async fn sync_progress(client: &mut NodeConn) -> Result<SyncProgress, BlockfrostError> {
    ...
    async fn action(
        ...
        let epoch =
            localstate::queries_v16::get_block_epoch_number(generic_client, current_era).await?;
        ...
        let wellknown_genesis = wellknown::GenesisValues::from_magic(genesis.network_magic.into())
            .ok_or_else(|| {

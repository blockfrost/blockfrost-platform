...
impl NodeConn {
    ...
    pub fn try_decode_error(&mut self, buffer: &[u8]) -> Result<Option<TxValidationError>, Error> {
...
mod tests {
    use super::*;
    use pallas_network::{facades::NodeClient, multiplexer::Bearer};

    #[tokio::test]
    async fn test_try_decode_error() {
        let buffer = [
        ...
        let bearer = Bearer::connect_tcp("127.0.0.1:6000")
            .await
            .expect("Failed to connect TCP Bearer");

        let mut node_conn = NodeConn {
            underlying: Some(NodeClient::new(bearer)),
        };

        let error = node_conn.try_decode_error(&buffer).unwrap();
